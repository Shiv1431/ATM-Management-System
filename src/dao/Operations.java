...
public class Operations {
	...
	DatabaseManagement manage;
	FailedTransaction fail;
	SuccessMessage success;
	
	Operations() throws SQLException {
		manage = new DatabaseManagement();
	...
	public void opView(String str, int id) throws SQLException {
		...
		sbt.setFont(new Font("Tahoma", Font.BOLD, 25));
		frame.add(sbt);
		...
		if (str.equals("Balance")){
			...
			try {
				...
				bal.setFont(new Font("Tahoma", Font.BOLD, 25));
				frame.add(bal);
			} catch (SQLException e1) {
				e1.printStackTrace();
			}
